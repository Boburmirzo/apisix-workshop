...
* *Step 2.* Setup `extra_lua_path` path to load your own plugin code. Add below line to
...
and write a code to print as an output the plug-in configuration data and request-related data information to the APISIX `error.log` file use the following code to log:
====
[source,lua]
...
.See the `file-logger.lua` file source code
[%collapsible]
====
[source,bash]
----
include::../custom-plugin/file-logger.lua[]
----
====
